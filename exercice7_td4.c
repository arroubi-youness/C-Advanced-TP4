...
Processeurs \n 3: Barettes memoire \n 4: Carte graphique \n----> ");
scanf("%d",&P[i].Type);
}while(P[i].Type != 1 & P[i].Type != 2 & P[i].Type != 3 &
P[i].Type != 4);
printf("Quantite: ");
scanf("%d",&P[i].Qte);
printf("Prix: ");
...
if(T[i].Type == j){
while(T[i].Qte < qte){
printf("Malheureusement, La 
quantite disponible est insufisant ! \n");
printf("Quelle est la quantite 
? \n----> ");
scanf("%d",&qte);
}
...
printf(" (: Notre boutique souhaite 
la bienvenue :) \n");
printf("Le produit demander est: ");
if(T[i].Type == 1) printf("Carte mere 
\n");
else if(T[i].Type == 2) 
printf("Processeur \n");
else if(T[i].Type == 3) 
printf("Barrettes memoire \n");
else if(T[i].Type == 4) printf("Cartes 
graphique \n");
printf("---> La Reference du 
produit: %d \n", T[i].Ref);
printf("---> Le prix totoal est: %.2f 
DH \n", qte*T[i].Prix);
}
}
int main(){
